use crate::binary::ion_cursor::BinaryIonCursor;
use crate::binary::ion_cursor::IonDataSource;
use crate::ion_system::IonSystem;
use crate::result::*;
...
impl<D: IonDataSource> BinaryIonReader<D> {
    ...
    fn read_symbol_table_imports(&mut self) -> IonResult<()> {
        Ok(())
    }
        ...
        while let Some(ion_type) = self.next()? {
            if ion_type != IonType::String {
                return decoding_error(format!(
                    "Found a {:?} value in the $ion_symbol_table symbols list",
                    ion_type
                ));
            }
    ...
    pub fn ion_dom_value(&mut self) -> IonResult<IonDomValue> {
        ...
        if self.is_null() {
            let dom_value =
                IonDomValue::new(field, annotations, IonValue::Null(IonNull::from(ion_type)));
            return Ok(dom_value);
        }
        ...
        let ion_value = match ion_type {
            Null => {
                unreachable!("Cannot have a null-typed value for which is_null() returns false.")
            }
    ...
    pub fn annotations(&self) -> IonResult<Vec<IonSymbol>> {
        ...
        for annotation_id in annotation_ids {
            let text: Rc<String> = match self.symbols.resolve(annotation_id) {
                ...
                None => {
                    return decoding_error(format!(
                        "Could not resolve annotation symbol ID {:?}",
                        annotation_id
                    ))
                }
            };
    ...
    pub fn field(&self) -> IonResult<Option<IonSymbol>> {
        ...
        match self.symbols.resolve(field_id) {
            ...
            None => decoding_error(format!("Could not resolve field symbol ID {:?}", field_id)),
        }
    }
        ...
        match ion_type {
            ...
            IonType::Symbol => {
                ...
                match self.symbols.resolve(symbol_id) {
                    ...
                    None => decoding_error(format!("Could not resolve symbol ID {:?}", symbol_id)),
                }
            }
    ...
    pub fn ion_dom_values<'a>(&'a mut self) -> impl Iterator<Item = IonResult<IonDomValue>> + 'a {
        BinaryIonCursorDomValues { reader: self }
    }
}
...
impl<'a, R> Iterator for BinaryIonCursorDomValues<'a, R>
where
    R: IonDataSource,
{
...
#[cfg(test)]
mod tests {
    use crate::binary::BinaryIonReader;
    ...
    #[test]
    fn test_read_timestamps() -> IonResult<()> {
        let mut buffer = create_ion_buffer();
        buffer.extend_from_slice(&[0x68, 0x80, 0x0F, 0xD0, 0x81, 0x81, 0x80, 0x80, 0x80]);

        let mut reader = BinaryIonReader::new(io::Cursor::new(buffer))?;
        let ion_type = reader.next()?.unwrap();
        assert_eq!(ion_type, IonType::Timestamp);
        let timestamp = reader.read_timestamp()?.unwrap();
        assert_eq!(timestamp.year(), 2000);
