...
impl<W: Write> BinaryIonWriter<W> {
    ...
    pub fn write_i64(&mut self, value: i64) -> IonResult<()> {
        ...
        println!(
            "Writing i64 value {}, size in bytes {}",
            value, size_in_bytes
        );
...
mod tests {
    ...
    fn expect<R: IonDataSource, F: Debug + PartialEq>(
        reader: &mut BinaryIonReader<R>,
        ion_type: IonType,
    ...
    fn expect_i64<R: IonDataSource>(reader: &mut BinaryIonReader<R>, value: i64) {
    ...
    fn test_round_trip_null() {
        ...
        let mut reader = BinaryIonReader::new(io_cursor).expect("Couldn't create reader.");
